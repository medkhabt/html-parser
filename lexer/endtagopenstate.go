...
func (s EndTagOpenState) nextToken() *token.Token {
	...
	if s.lexer.ch == '>' {
		...
		s.lexer.state = TagNameState{s.lexer, &token.Token{token.ENDTAG, []byte{s.lexer.ch}}}
		return s.lexer.state.nextToken()
	} else if s.lexer.ch >= 'A' && s.lexer.ch <= 'Z' {
		s.lexer.state = TagNameState{s.lexer, &token.Token{token.ENDTAG, []byte{s.lexer.ch + byte(0x20)}}}
		return s.lexer.state.nextToken()
	} else {
